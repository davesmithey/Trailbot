...
def extract_trail_run_distances(page_text):
    """Extract trail run race distances from page text"""
    distances = []

    # Look for "Trail Runs:" section and extract distances until the next race type or end
    trail_match = re.search(
        r'Trail\s+Runs:\s*(.+?)(?=Cross\s+Country|When:|Where:|$)',
        page_text,
        re.IGNORECASE | re.DOTALL
    )

    if trail_match:
        trail_text = trail_match.group(1)
        # Extract all distance patterns (including Youth which doesn't have a number)
        distance_patterns = [
            r'(\d+\s*(?:k|km|mi|mile|miles))',  # Numbered distances like "50K", "25mi"
            r'(Youth)',  # Youth race (no number)
        ]

        for pattern in distance_patterns:
            matches = re.finditer(pattern, trail_text, re.IGNORECASE)
...
def extract_mountain_bike_distances(page_text):
    """Extract mountain bike race distances from page text"""
    distances = []

    # Look for "Cross Country Mountain Bike:" section and extract distances
    bike_match = re.search(
        r'Cross\s+Country\s+Mountain\s+Bike:\s*(.+?)(?=When:|Where:|$)',
        page_text,
        re.IGNORECASE | re.DOTALL
    )

    if bike_match:
        bike_text = bike_match.group(1)
        # Extract all distance patterns
        distance_patterns = [
            r'(\d+\s*(?:k|km|mi|mile|miles))',
        ]

        for pattern in distance_patterns:
            matches = re.finditer(pattern, bike_text, re.IGNORECASE)
            for match in matches:
                distance_str = match.group(0).strip()
                if distance_str not in distances:
                    distances.append(distance_str)
...
def extract_distances(page_text):
    """Extract all race distances from page text (backwards compatibility)"""
    trail_distances = extract_trail_run_distances(page_text)
    bike_distances = extract_mountain_bike_distances(page_text)
    return {
        'trail_runs': trail_distances,
        'mountain_bike': bike_distances
    }
...
def update_knowledge_base(kb, scraped_data):
    ...
    if 'rivers_edge_content' in scraped_data:
        ...
        # Extract distances by race type
        distance_data = extract_distances(new_content)

        # Extract trail run distances
        trail_distances = distance_data.get('trail_runs', [])
        if trail_distances and kb['race'].get('distances') != trail_distances:
            kb['race']['distances'] = trail_distances
            changes_made = True
            print(f"✓ Extracted trail run distances: {', '.join(trail_distances)}")

        # Extract mountain bike distances
        bike_distances = distance_data.get('mountain_bike', [])
        if 'mountain_bike' not in kb:
            kb['mountain_bike'] = {}
        if bike_distances and kb['mountain_bike'].get('distances') != bike_distances:
            kb['mountain_bike']['distances'] = bike_distances
            changes_made = True
            print(f"✓ Extracted mountain bike distances: {', '.join(bike_distances)}")
